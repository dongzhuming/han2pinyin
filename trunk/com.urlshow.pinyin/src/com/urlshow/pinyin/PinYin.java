...
public abstract class PinYin {
		...
		if (HAN_ZI == null) {
			...
			try {
				br = new BufferedReader(new InputStreamReader(
						PinYin.class.getResourceAsStream("hanzi.txt"), "UTF-8"));
				HAN_ZI = br.readLine();
			} catch (IOException e) {
				HAN_ZI = "";
			} finally {
	...
	private static String zhuYin(String str) {
		...
		if (i == -1)
			return str;
		return getHAN_ZI().substring(getHAN_ZI().indexOf("[", i) + 1,
				getHAN_ZI().indexOf("]", i));
	}
	 ...
	 *             字符集错误
	 */
	public static String getPinYin(String str, int type, String charsetName)
			throws UnsupportedEncodingException {
		if (charsetName == null || charsetName.length() == 0
				|| charsetName.toLowerCase().trim().equals("utf-8"))
		...
		case CI_ZU:
			for (int i = 0; i < str.length(); i++) {
				if (i > 0) {
					...
					String temp = zhuYin(str.substring(i, i + 1)).substring(0,
							1);
